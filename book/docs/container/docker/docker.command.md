...
将镜像nginx标记为 taotaolinux_nginx 镜像。
```
[root@localhost ~]# docker images
REPOSITORY          TAG                 IMAGE ID            CREATED             SIZE
nginx               latest              bb776ce48575        3 days ago          109MB
[root@localhost ~]# docker tag nginx taotaolinux_nginx
[root@localhost ~]# docker images
REPOSITORY          TAG                 IMAGE ID            CREATED             SIZE
taotaolinux_nginx    latest              bb776ce48575        3 days ago          109MB
nginx               latest              bb776ce48575        3 days ago          109MB
...
taotaolinux_nginx    latest              bb776ce48575        3 days ago          109MB
nginx               latest              bb776ce48575        3 days ago          109MB
[root@localhost ~]# docker run -itd -p88:80 taotaolinux_nginx
8670f902dc094eb1de78eeccac90a4618ab59a8ba787e6da6324589c1039daa7
[root@localhost ~]# docker ps
CONTAINER ID        IMAGE               COMMAND                  CREATED             STATUS              PORTS                NAMES
8670f902dc09        taotaolinux_nginx    "nginx -g 'daemon of…"   2 seconds ago       Up 2 seconds        0.0.0.0:88->80/tcp   compassionate_gates
[root@localhost ~]# docker export 8670f902dc09 > taotaolinux_nginx.tar
[root@localhost ~]# du -sh taotaolinux_nginx.tar 
107M	taotaolinux_nginx.tar
```
导出后在本地可以看到有一个`taotaolinux_nginx.tar`的容器快照。

导入某个容器，有了容器快照之后，可以在想要的时候随时导入。导入快照使用docker import命令。

```
[root@localhost ~]# docker images
REPOSITORY          TAG                 IMAGE ID            CREATED             SIZE
nginx               latest              bb776ce48575        3 days ago          109MB
[root@localhost ~]# cat taotaolinux_nginx.tar | docker import - taotaolinux_nginx 
sha256:a2855f206c05c76a2ef1bc369b24dded80df110bf4fef27e2f5107633be1226b
[root@localhost ~]# docker images
REPOSITORY          TAG                 IMAGE ID            CREATED             SIZE
taotaolinux_nginx    latest              a2855f206c05        1 second ago        108MB
nginx               latest              bb776ce48575        3 days ago          109MB
```
