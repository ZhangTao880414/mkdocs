...
none|-net=none,获取独立的network namespace，但不为容器进行任何网络配置，需要手动配置。
...
<center>![桥接模式](../../pictures/container/docker/p4.png)</center>
...
<center>![host模式](../../pictures/container/docker/p5.png)</center>
...
<center>![container模式](../../pictures/container/docker/p6.png)</center>

```
docker run -itd  --name taotaolinux busybox
docker run -tid --net=container:taotaolinux --name docker_con1 busybox
```
> 两个docker容器，使用一个网络
...
??? note "操作"
    ...
    [root@localhost ~]# docker run -itd  --name taotaolinux busybox
    b1c62a22bfc9db38b69d234b0ba0745dd7a0a206b47e6420e442547990026162
    [root@localhost ~]# docker ps -l
    CONTAINER ID        IMAGE               COMMAND             CREATED             STATUS              PORTS               NAMES
    b1c62a22bfc9        busybox             "sh"                5 seconds ago       Up 4 seconds                            taotaolinux
    [root@localhost ~]# docker run -tid --net=container:taotaolinux --name docker_con1 busybox
...
### 1.3 none
&#160; &#160; &#160; &#160;使用none模式，Docker容器拥有自己的Network Namespace，但是，并不为Docker容器进行任何网络配置。也就是说，这个Docker容器没有网卡、IP、路由等信息。需要自己为Docker容器添加网卡、配置IP等。

<center>![container模式](../../pictures/container/docker/p7.png)</center>
...
docker network create taotaolinux_network
docker run -itd --name web1 --net=taotaolinux_network busybox
docker run -itd --name db1 --net=taotaolinux_network busybox
...
??? note "操作"
    ...
    [root@localhost ~]# docker network create taotaolinux_network
    e08426e96c48d072c108352f43fb88134e1e451fb40eddf1be541cb25f11ad66
    [root@localhost ~]# docker network ls
    NETWORK ID          NAME                DRIVER              SCOPE
    fded918d7084        bridge              bridge              local
    e08426e96c48        taotaolinux_network         bridge              local
    7fb098cc4817        host                host                local
    bd86d4f496b9        none                null                local
    
    [root@localhost ~]# docker run -itd --name web1 --net=taotaolinux_network busybox
    607659807fbdf5f07849e496218c0f4c5b8fc65211f21d5098498cbaa3d4fa16
    
    [root@localhost ~]# docker run -itd --name db1 --net=taotaolinux_network busybox
...
??? note "操作"
    ```
    [root@localhost ~]# docker network inspect taotaolinux_network 
    [
        {
            "Name": "taotaolinux_network",
...
![网络访问原理](../../pictures/container/docker/network.png)

![网络访问原理](../../pictures/container/docker/net1.png)

![网络访问原理](../../pictures/container/docker/net2.png)
