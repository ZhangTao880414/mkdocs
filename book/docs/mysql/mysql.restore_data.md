...
## 1. binlog介绍
&#160; &#160; &#160; &#160; 众所周知，binlog日志对于mysql数据库来说是十分重要的。在数据丢失的紧急情况下，往往会想到用binlog日志功能进行数据恢复（定时全备份+binlog日志恢复增量数据部分），为你一路保驾护航！
...
*************************** 5. row ***************************
 ...
 Event_type: Rows_query
  Server_id: 1
End_log_pos: 389
       Info: # insert into taotaolinux(name,age,score) values("taotaolinux1 from leco data",40,88)
*************************** 6. row ***************************
   Log_name: mysql-bin.000007
        Pos: 389
 Event_type: Table_map
  Server_id: 1
End_log_pos: 442
       Info: table_id: 109 (summer.taotaolinux)
*************************** 7. row ***************************
   Log_name: mysql-bin.000007
        Pos: 442
...
insert into taotaolinux(name,age,score) values("taotaolinux1",18,88);
insert into taotaolinux(name,age,score) values("taotaolinux2",28,88);
insert into taotaolinux(name,age,score) values("taotaolinux3",38,88);
insert into taotaolinux(name,age,score) values("taotaolinux4",48,88);

select * from taotaolinux;
```
执行过程
...
ERROR 1051 (42S02): Unknown table 'summer.taotaolinux'
mysql> create table taotaolinux(
    -> id int not null primary key auto_increment,
    -> name varchar(20),
    -> age int not null,
    -> score int not null
...
mysql>
mysql> insert into taotaolinux(name,age,score) values("taotaolinux1",18,88);
Query OK, 1 row affected (0.01 sec)

mysql> insert into taotaolinux(name,age,score) values("taotaolinux2",28,88);
Query OK, 1 row affected (0.02 sec)

mysql> insert into taotaolinux(name,age,score) values("taotaolinux3",38,88);
Query OK, 1 row affected (0.03 sec)

mysql> insert into taotaolinux(name,age,score) values("taotaolinux4",48,88);
Query OK, 1 row affected (0.01 sec)

mysql>
mysql> select * from taotaolinux;
+----+------+-----+-------+
| id | name | age | score |
+----+------+-----+-------+
|  1 | taotaolinux1 |  18 |    88 |
|  3 | taotaolinux2 |  28 |    88 |
|  5 | taotaolinux3 |  38 |    88 |
|  7 | taotaolinux4 |  48 |    88 |
+----+------+-----+-------+
...
### 5.2 开工
&#160; &#160; &#160; &#160;早上开始，由于业务的需求会对数据库进行各种“增删改”操作。比如在taotaolinux表中做了增删改查操作。
快速命令
```
insert into taotaolinux(name,age,score) values("taotaolinux5",58,88);
insert into taotaolinux(name,age,score) values("taotaolinux6",68,88);
update taotaolinux set age=100 where name='taotaolinux1';
update taotaolinux set age=200 where name='taotaolinux2';
```
操作过程
```
mysql> use summer;
...
mysql> insert into taotaolinux(name,age,score) values("taotaolinux5",58,88);
Query OK, 1 row affected (0.01 sec)

mysql> insert into taotaolinux(name,age,score) values("taotaolinux6",68,88);
Query OK, 1 row affected (0.01 sec)
...
|  5 | taotaolinux3 |  38 |    88 |
|  7 | taotaolinux4 |  48 |    88 |
|  9 | taotaolinux5 |  58 |    88 |
| 11 | taotaolinux6 |  68 |    88 |
+----+------+-----+-------+
...
mysql> update taotaolinux set age=100 where name='taotaolinux1';
Query OK, 1 row affected (0.01 sec)
Rows matched: 1  Changed: 1  Warnings: 0

mysql> update taotaolinux set age=200 where name='taotaolinux2';
Query OK, 1 row affected (0.01 sec)
Rows matched: 1  Changed: 1  Warnings: 0
...
| id | name | age | score |
+----+------+-----+-------+
|  1 | taotaolinux1 | 100 |    88 |
|  3 | taotaolinux2 | 200 |    88 |
|  5 | taotaolinux3 |  38 |    88 |
|  7 | taotaolinux4 |  48 |    88 |
|  9 | taotaolinux5 |  58 |    88 |
| 11 | taotaolinux6 |  68 |    88 |
...
```
接着执行一次刷新日志索引操作，重新开始新的binlog日志记录文件。按理说mysql-bin.0000013，这个文件不会再有后续写入了，因为便于分析原因及查找ops节点，以后所有数据库操作都会写入到下一个日志文件。
...
*************************** 5. row ***************************
 ...
 Event_type: Rows_query
  Server_id: 1
End_log_pos: 374
       Info: # insert into taotaolinux(name,age,score) values("taotaolinux5",58,88)
*************************** 6. row ***************************
   Log_name: mysql-bin.000013
        Pos: 374
 Event_type: Table_map
  Server_id: 1
End_log_pos: 427
       Info: table_id: 133 (summer.taotaolinux)
*************************** 7. row ***************************
   Log_name: mysql-bin.000013
        Pos: 427
 Event_type: Write_rows
...
*************************** 11. row ***************************
 ...
 Event_type: Rows_query
  Server_id: 1
End_log_pos: 731
       Info: # insert into taotaolinux(name,age,score) values("taotaolinux6",68,88)
*************************** 12. row ***************************
   Log_name: mysql-bin.000013
        Pos: 731
 Event_type: Table_map
...
*************************** 17. row ***************************
 ...
 Event_type: Rows_query
  Server_id: 1
End_log_pos: 1076
       Info: # update taotaolinux set age=100 where name='taotaolinux1'
*************************** 18. row ***************************
   Log_name: mysql-bin.000013
        Pos: 1076
 Event_type: Table_map
...
*************************** 23. row ***************************
 ...
 Event_type: Rows_query
  Server_id: 1
End_log_pos: 1440
       Info: # update taotaolinux set age=200 where name='taotaolinux2'
*************************** 24. row ***************************
   Log_name: mysql-bin.000013
        Pos: 1440
 Event_type: Table_map
...
*************************** 28. row ***************************
 ...
 Event_type: Query
  Server_id: 1
End_log_pos: 1786
       Info: use `summer`; DROP TABLE `taotaolinux` /* generated by server */
*************************** 29. row ***************************
   Log_name: mysql-bin.000013
        Pos: 1786
 Event_type: Rotate
...
--------------
DROP TABLE IF EXISTS `taotaolinux`
--------------
...
--------------
LOCK TABLES `taotaolinux` WRITE
--------------

--------------
/*!40000 ALTER TABLE `taotaolinux` DISABLE KEYS */
--------------

--------------
INSERT INTO `taotaolinux` VALUES (1,'taotaolinux1',18,88),(3,'taotaolinux2',28,88),(5,'taotaolinux3',38,88),(7,'taotaolinux4',48,88)
--------------

--------------
/*!40000 ALTER TABLE `taotaolinux` ENABLE KEYS */
--------------
...
| id | name | age | score |
+----+------+-----+-------+
|  2 | taotaolinux1 |  18 |    88 |
|  4 | taotaolinux2 |  28 |    88 |
|  6 | taotaolinux3 |  38 |    88 |
|  8 | taotaolinux4 |  48 |    88 |
+----+------+-----+-------+
4 rows in set (0,00 sec)
...
root@leco:/mysqldataback# # DROP TABLE `taotaolinux` /* generated by server */  删除这个语句
...
-rw-r-----  1 root root 1833 3月   1 17:34 mysql-bin.000013     # 最初删除drop table taotaolinux，就是记录在这个binlog中的。
...
No query specified
```
从上面可以看到。在drop的时候。mysql的pos开始至1661结束1786，也就是恢复到1661，即可。
...
mysql: [Warning] Using a password on the command line interface can be insecure.
root@leco:/mysqldataback# mysql -uroot -proot -e 'use summer;select * from taotaolinux;'
mysql: [Warning] Using a password on the command line interface can be insecure.
+----+------+-----+-------+
| id | name | age | score |
+----+------+-----+-------+
