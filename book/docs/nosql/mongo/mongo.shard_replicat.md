...
![集群图](../../pictures/nosql/mongo/集群图.png)
...
### 4.2 测试
开始测试复制集和分片集，使用预先分片,手动预先分片是为了防止未来chunk的移动，减少IO。
...
sh.enableSharding("taotaolinux");
sh.shardCollection('taotaolinux.goods',{'goods_id':1});
for (var i=1;i<=30000;i++){
    db.goods.insert({goods_id:i,good_name:'summer'});
}

# 9. 手动预先分片
sh.shardCollection('taotaolinux.taotaolinux',{'userid':1});
# 先分块,先分40个块，预先在1K,2K,...40K这样的界限切好chunk（虽然chunk还是空的）
# 但是chunk会均匀的移动到各片上
for(var i=1;i<=40;i++){
    sh.splitAt('taotaolinux.taotaolinux',{userid:i*1000})
}
# 插入数据
for(var i=1;i<=10000;i++){
    db.taotaolinux.insert({userid:i,name:'taotaolinux'+i})
}

sh.shardCollection("taotaolinux.users",{"userId": 1 })
for(var i=1; i<=30; i++){ sh.splitAt("taotaolinux.users", {userId: i*1000}) }
...
for(var i=1; i<30000; i++){ db.users.insert({userId: i,name: 'taotaolinux'})};
```
...
```
从上面可以看出数据默认是在shard1上的。接下来要设置shop库是要分片的。[也就是partitioned=true]
...
```
mongos> for(var i=1; i<30000; i++){ db.users.insert({userId: i,name: 'taotaolinux'})};
