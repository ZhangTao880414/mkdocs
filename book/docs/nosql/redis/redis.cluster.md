...
### 1.2 节点分配
&#160; &#160; &#160; &#160;现在是三个主节点分别是：A, B, C 三个节点，它们可以是一台机器上的三个端口，也可以是三台不同的服务器。那么，采用哈希槽 (hash slot)的方式来分配16384个slot 的话，它们三个节点分别承担的slot 区间是：
...
&#160; &#160; &#160; &#160;上面那个例子里, 集群有ABC三个主节点, 如果这3个节点都没有加入从节点，如果B挂掉了，就无法访问整个集群了。A和C的slot也无法访问。
&#160; &#160; &#160; &#160; 所以在集群建立的时候，一定要为每个主节点都添加了从节点, 比如像这样, 集群包含主节点A、B、C, 以及从节点A1、B1、C1, 那么即使B挂掉系统也可以继续正确工作。B1节点替代了B节点，所以Redis集群将会选择B1节点作为新的主节点，集群将会继续正确地提供服务。 当B重新开启后，它就会变成B1的从节点。
...
### 2.2 配置 
```
root@manage01:/usr/local/taotaolinux# tree .
.
├── redis01
│   ├── bin
│   │   ├── redis-benchmark
...
root@manage01:/usr/local/taotaolinux# tree ./ -L 2
./
├── redis01
│   ├── bin
│   ├── conf
...
```
root@manage01:/usr/local/taotaolinux# cat redis01/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis01/log/redis.pid"
port 7001
timeout 300
cluster-enabled yes
cluster-config-file /usr/local/taotaolinux/redis01/nodes.conf
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis01/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis01/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# cat redis02/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis02/log/redis.pid"
port 7002
timeout 300
cluster-enabled yes
cluster-config-file '/usr/local/taotaolinux/redis02/nodes.conf'
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis02/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis02/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# cat redis03/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis03/log/redis.pid"
port 7003
timeout 300
cluster-enabled yes
cluster-config-file '/usr/local/taotaolinux/redis03/nodes.conf'
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis03/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis03/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# cat redis04/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis04/log/redis.pid"
port 7004
timeout 300
cluster-enabled yes
cluster-config-file '/usr/local/taotaolinux/redis04/nodes.conf'
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis04/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis04/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# cat redis05/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis05/log/redis.pid"
port 7005
timeout 300
cluster-enabled yes
cluster-config-file '/usr/local/taotaolinux/redis05/nodes.conf'
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis05/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis05/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# cat redis06/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis06/log/redis.pid"
port 7006
timeout 300
cluster-enabled yes
cluster-config-file '/usr/local/taotaolinux/redis06/nodes.conf'
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis06/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis06/data"
appendonly yes
appendfsync always
bind 0.0.0.0
```
...
```
root@manage01:/usr/local/taotaolinux# pkill redis
root@manage01:/usr/local/taotaolinux# ps axf|grep redis
 9716 pts/18   S+     0:00                          \_ grep --color=auto redis
root@manage01:/usr/local/taotaolinux# cat allstart.sh
#!/bin/bash
...
root@manage01:/usr/local/taotaolinux# sh allstart.sh
root@manage01:/usr/local/taotaolinux# ps axu|grep redis
...
```
root@manage01:/usr/local/taotaolinux# apt-get install ruby
```

使用redis-trib.rb创建集群，先拷贝创建集群的命令过来

```
root@manage01:/usr/local/taotaolinux# cp /usr/local/redis-5.0.3/src/redis-trib.rb .  # 从源码包中拷贝
root@manage01:/usr/local/taotaolinux# l
...
```
root@manage01:/usr/local/taotaolinux# ./redis-trib.rb create --replicas 1 127.0.0.1:7001 127.0.0.1:7002 127.0.0.1:7003 127.0.0.1:7004 127.0.0.1:7005 127.0.0.1:7006
...
root@manage01:/usr/local/taotaolinux# redis-cli --cluster create 127.0.0.1:7001 127.0.0.1:7002 127.0.0.1:7003 127.0.0.1:7004 127.0.0.1:7005 127.0.0.1:7006 --cluster-replicas 1
...
```
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7001
127.0.0.1:7001> set name taotaolinux
...
127.0.0.1:7002>
```
根据redis-cluster的key值分配，name应该分配到节点7002[5461-10922]上，上面显示redis cluster自动从7001跳转到了7002节点。可以测试一下7006从节点获取name值

```
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7006
127.0.0.1:7006> get name
-> Redirected to slot [5798] located at 127.0.0.1:7002
"taotaolinux"
127.0.0.1:7002>
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7005
127.0.0.1:7005> get name
-> Redirected to slot [5798] located at 127.0.0.1:7002
"taotaolinux"
127.0.0.1:7002>
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7004
127.0.0.1:7004> get name
-> Redirected to slot [5798] located at 127.0.0.1:7002
"taotaolinux"
127.0.0.1:7002>
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7003
127.0.0.1:7003> get name
-> Redirected to slot [5798] located at 127.0.0.1:7002
"taotaolinux"
127.0.0.1:7002>
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7002
127.0.0.1:700
```
...
```
root@manage01:/usr/local/taotaolinux# ./redis-cli --cluster check 127.0.0.1:7002
...
root@manage01:/usr/local/taotaolinux# kill 9729
root@manage01:/usr/local/taotaolinux# ps axf|grep redis
...
root@manage01:/usr/local/taotaolinux# ./redis-cli --cluster check 127.0.0.1:7005
Could not connect to Redis at 127.0.0.1:7002: Connection refused
...
[OK] All 16384 slots covered.
```
可以看到集群连接不了7002节点，而7005有原来的S转换为M节点，代替了原来的7002节点。可以获取name值：
...
### 4.2 恢复7002节点
 现在将7002节点恢复，看是否会自动加入集群中以及充当的M还是S节点。
 
```
root@manage01:/usr/local/taotaolinux# ./redis02/bin/redis-server redis02/conf/redis.conf
root@manage01:/usr/local/taotaolinux# !ps
ps axf|grep redis
...
```
root@manage01:/usr/local/taotaolinux# cat redis07/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis07/log/redis.pid"
port 7007
timeout 300
cluster-enabled yes
cluster-config-file /usr/local/taotaolinux/redis07/nodes.conf
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis07/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis07/data"
appendonly yes
appendfsync always
bind 0.0.0.0
root@manage01:/usr/local/taotaolinux# ./redis07/bin/redis-server redis07/conf/redis.conf
root@manage01:/usr/local/taotaolinux# ps axf|grep redis
...
```
root@manage01:/usr/local/taotaolinux# ./redis07/bin/redis-server redis07/conf/redis.conf
root@manage01:/usr/local/taotaolinux# ./redis-cli --cluster add-node 127.0.0.1:7007 127.0.0.1:7002
...
```
root@manage01:/usr/local/taotaolinux# ./redis-cli --cluster check 127.0.0.1:7007
...
```
root@manage01:/usr/local/taotaolinux# ./redis-cli -c -p 7007
...
redis-cluster在新增节点时并未分配卡槽，需要手动对集群进行重新分片迁移数据，需要重新分片命令 reshard
...
```
root@manage01:/usr/local/taotaolinux# ./redis-cli --cluster reshard 127.0.0.1:7005
[OK] All nodes agree about slots configuration.  
>>> Check for open slots...  
>>> Check slots coverage...  
[OK] All 16384 slots covered.  
...
```
它提示需要迁移多少slot到7007上，平分16384个哈希槽给4个节点：16384/4 = 4096，需要移动4096个槽点到7007上。
...
如果不打算从特定的节点上取出指定数量的哈希槽，那么可以向redis-trib输入 all，这样的话， 集群中的所有主节点都会成为源节点，redis-trib从各个源节点中各取出一部分哈希槽，凑够4096个，然后移动到7007节点上：

```
Source node #1:all  
```
 确认之后，redis-trib就开始执行分片操作，将哈希槽一个一个从源主节点移动到7007目标主节点。重新分片结束后可以check以下节点的分配情况。
...
```
root@manage01:/usr/local/taotaolinux# cp -aR redis01 redis08
root@manage01:/usr/local/taotaolinux# rm redis08/data/nodes.conf
root@manage01:/usr/local/taotaolinux# vim redis08/conf/redis.conf
root@manage01:/usr/local/taotaolinux# cat !$
cat redis08/conf/redis.conf
daemonize yes
pidfile "/usr/local/taotaolinux/redis08/log/redis.pid"
port 7008
timeout 300
cluster-enabled yes
cluster-config-file /usr/local/taotaolinux/redis08/nodes.conf
cluster-node-timeout 15000
loglevel debug
logfile "/usr/local/taotaolinux/redis08/log/redis.log"
databases 16
save 900 1
save 300 10
save 60 10000
rdbcompression yes
dbfilename "dump.rdb"
dir "/usr/local/taotaolinux/redis08/data"
appendonly yes
appendfsync always
bind 0.0.0.0
```
...
```
root@manage01:/usr/local/taotaolinux# ./redis08/bin/redis-server redis08/conf/redis.conf
root@manage01:/usr/local/taotaolinux# !ps
ps axf|grep redis
